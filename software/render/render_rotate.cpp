...
fixed_point_t *load_matrix(float angle) {
...
//    glm::mat4 Projection = glm::perspective(
  //      glm::radians(45.0f), (float)640 / (float)480, 0.1f, 100.0f);
   ...
   glm::mat4 Projection = glm::ortho(-10.0f,10.0f,-10.0f,10.0f,0.0f,100.0f);
    ...
    // Camera matrix
    glm::mat4 rotation = glm::rotate((glm::mediump_float)angle, glm::vec3(0, 1, 0));
    glm::vec4 lookat = rotation * glm::vec4(4, 3, 3, 1);
    glm::mat4 View = glm::lookAt(
        glm::vec3(lookat.x, lookat.y, lookat.z),  // Camera is at (4,3,3), in World Space
   ...
   // glm::vec4 test = mvp * glm::vec4(0, 0, -8, 1);
    cout << "rasterizer: reference = " << glm::to_string(mvp) << endl;
    cout << "lookat = " << glm::to_string(lookat) << endl;
...
int main(){
  ...
  *(int*)(map_render+4) = vertex_buffer_offset;
  ...
  for(;;){
    angle = i%360;
    fixed_point_t*mat = load_matrix(angle);
